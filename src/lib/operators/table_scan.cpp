...
template <typename T>
std::shared_ptr<const Table> TableScan::TableScanImpl<T>::scan(std::shared_ptr<const Table> table,
  ...
  for (auto chunk_id = ChunkID(0); chunk_id < table->chunk_count(); ++chunk_id) {
    ...
    DebugAssert(dict_column,
                "Unknown Column Type in Table Scan");  // make sure the above is the case; only in debug mode
    ...
    if (comp_value != ValueID{0} && comp_value != INVALID_VALUE_ID &&
        (scan_type == ScanType::OpLessThanEquals || scan_type == ScanType::OpGreaterThan)) {
      comp_value--;
    }
